...
def handle_task_action(task_id, action, reason):
    ...
    if action == 'stop':
        task['state'] = 'Blocked'
        task['block'] = reason or '管理员暂停'
        task['now'] = f'⏸️ 已暂停：{reason}'
    elif action == 'cancel':
        task['state'] = 'Cancelled'
        task['block'] = reason or '管理员取消'
    ...
    if action == 'resume':
        ...
        _scheduler_add_flow(task, f'管理员{action}：{reason or "无"}')
...
def handle_create_task(title, org='产品经理', official='产品总监', priority='normal', template_id='', params=None, target_dept=''):
    ...
    # 清理常见前缀: "创建任务:" 等前缀
    title = re.sub(r'^创建任务[：:\uff1a]\s*', '', title)
    if len(title) > 100:
        title = title[:100] + '…'
    # 标题质量校验：防止闲聊被误建为任务
    if len(title) < _MIN_TITLE_LEN:
        return {'ok': False, 'error': f'标题过短（{len(title)}<{_MIN_TITLE_LEN}字），不像是有效任务'}
    ...
    # 正确流程起点：管理员 -> 秘书分拣
    # target_dept 记录模板建议的最终执行部门（仅供项目经理派发参考）
    initial_org = '秘书'
    new_task = {
        'id': task_id,
        'title': title,
    ...
    return {'ok': True, 'taskId': task_id, 'message': f'任务 {task_id} 已创建，正在派发给秘书'}
...
def handle_review_action(task_id, action, comment=''):
    ...
    if action == 'approve':
        if task['state'] == 'Menxia':
            task['state'] = 'Assigned'
            task['now'] = '审核通过，移交项目经理派发'
            remark = f'✅ 通过：{comment or "审核通过"}'
            to_dept = '项目经理'
        else:  # Review
            task['state'] = 'Done'
            task['now'] = '御批通过，任务完成'
            remark = f'✅ 审核通过：{comment or "审查通过"}'
            to_dept = '管理员'
    elif action == 'reject':
        round_num = (task.get('review_round') or 0) + 1
        task['review_round'] = round_num
        task['state'] = 'Zhongshu'
        task['now'] = f'驳回退回产品经理修订（第{round_num}轮）'
    ...
    task.setdefault('flow_log', []).append({
        'at': now_iso(),
        'from': '质量审核' if task.get('state') != 'Done' else '管理员',
        'to': to_dept,
        'remark': remark
    })
...
def handle_scheduler_rollback(task_id, reason=''):
    ...
    task['now'] = f'↩️ 调度系统自动回滚：{reason or "恢复到上个稳定节点"}'
    task['block'] = '无'
    sched['retryCount'] = 0
    sched['escalationLevel'] = 0
    sched['stallSince'] = None
...
def handle_repair_flow_order():
    ...
    for task in tasks:
        ...
        first = flow_log[0]
        if first.get('from') != '管理员' or first.get('to') != '产品经理':
            continue
        ...
        if task.get('state') == 'Zhongshu' and task.get('org') == '产品经理' and len(flow_log) == 1:
            task['state'] = 'Taizi'
            task['org'] = '秘书'
            task['now'] = '等待秘书分拣处理'
...
_STATE_FLOW = {
    ...
    'Next':     ('Doing', '项目经理', '各部门', '待执行任务开始执行'),
    'Doing':    ('Review', '各部门', '项目经理', '各部门完成，进入汇总'),
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    if agent_id is None and new_state in ('Doing', 'Next'):
        ...
        if target_dept and target_dept in _ORG_AGENT_MAP:
            ...
            log.info(f'📍 {task_id} 使用 target_dept={target_dept} → agent={agent_id}')
        else:
            # 回退到 org 字段
            org = task.get('org', '')
            agent_id = _ORG_AGENT_MAP.get(org)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p in ('', '/dashboard', '/dashboard.html'):
            # 优先使用单文件 dashboard.html，回退到 dist
            if DASHBOARD_HTML.exists():
                self.send_file(DASHBOARD_HTML)
            else:
    ...
    def do_POST(self):
        ...
        if p == '/api/task-action':
            ...
            reason = body.get('reason', '').strip() or f'管理员从看板{action}'
        ...
        if p == '/api/create-task':
            ...
            org = body.get('org', '产品经理').strip()
            official = body.get('official', '产品总监').strip()
