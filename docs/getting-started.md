...
> 从零开始，5 分钟搭建你的多Agent协作 AI 协同系统

---

## 第一步：安装 OpenClaw

多Agent协作基于 [OpenClaw](https://openclaw.ai) 运行，请先安装：
...
在 OpenClaw 中配置消息渠道（Feishu / Telegram / Signal），将 `taizi`（前台）Agent 设为任务入口。前台会自动分拣闲聊与指令，指令类消息提炼标题后转发产品经理。
